...
export const staffApi = {
  initializeUserSession: async function (): Promise<{
    ...
    try {
      ...
      let stationId = currentUser.stationId; 
  ...
  async getBatteries(stationId: number | string): Promise<Battery[]> {
    try {
      if (!stationId || stationId === 0 || stationId === '0') {
  ...
  async getQueue(stationId: number | string): Promise<Booking[]> {
    try {
  ...
  async completeSwapProcess(swapData: SwapData, staffId: string, stationId: number | string): Promise<void> {
    try {
  ...
  async getTransactions(stationId: number | string, limit: number = 10, date?: string): Promise<Transaction[]> {
    try {
  ...
  async getRevenueStats(stationId: number | string, period: 'monthly' | 'quarterly' | 'yearly' = 'monthly'): Promise<RevenueStats> {
    try {
      const periodLabels = {
  ...
  async getDailyStats(stationId: number | string, date?: string): Promise<DailyStats> {
    try {
  ...
  async createInspection(inspectionData: InspectionData, staffId: string, stationId: number | string): Promise<void> {
    try {
  ...
  async getStaffProfile(staffId: string): Promise<any> {
    try {
      ...
      try {
        ...
        if (response.data.role === 'Staff' || response.data.role === 'staff') {
          return {
            ...
            stationId: response.data.stationId, // Must be GUID from backend
      ...
      return {
        ...
        stationId: staffData.stationId, // Must be GUID from backend
